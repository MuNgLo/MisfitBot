...
namespace MisfitBot2.Services
{
    class MatchMakingService : ServiceBase, IService
    {
        ...
        private async Task<MMQueue> QueueData(ulong discordChannelID, int teamsize=5)
        {
        ...
        {
            MMQueue queue = await QueueData(discordChannelID);
        ...
        {
            MMQueue queue = await QueueData(discordChannelID);
            if(queue.currentstate != QUEUESTATE.PENDING) { return; }
            if (args.Count > 1)
            {
        ...
        {
            MMQueue queue = await QueueData(discordChannelID);
            if (!queue.active || queue.currentstate != QUEUESTATE.RECRUITING || user._discordUID == 0)
            {
                return false;
            }
        ...
        private async Task StartQueueInChannel(BotChannel bChan, ulong discordChannelID, List<string> args = null)
        {
            int teamsize = 5;
            if(args != null)
            {
        ...
        {
            MatchMakingSettings settings = await Settings(bChan);
            MMQueue queue = await QueueData(discordChannelID, teamsize);
            if (settings._active && !queue.active)
            {
                queue.active = true;
                SaveQueue(queue);
                await SayOnDiscord($"Matchmaking is now running a queue in this channel. Make sure to set the game with !mm game <gamename>", discordChannelID);
            }
        }
        private async Task SetQueueName(BotChannel bChan, ulong discordChannelID, List<string> args)
        {
            ...
            if(queue.game != game)
            {
                ...
                queue.currentstate = QUEUESTATE.RECRUITING;
                SaveQueue(queue);
                await SayOnDiscord($"This channels queue is now for the game {game}", discordChannelID);
            }
        }
        private async Task RunQueue(ulong discordChannelID)
        {
            ...
            if (queue.currentstate == QUEUESTATE.INACTIVE || queue.currentstate == QUEUESTATE.ENDED)
            {
                ...
                queue.teamB = new List<ulong>();
                SaveQueue(queue);
                await SayOnDiscord($"Queue is now open. Type '!signup' to participate.", discordChannelID);
            }
