...
using MisfitBot_MKII.Plugins.Admin;
...
namespace MisfitBot_MKII.Services
{
    class AdminService : ServiceBase, IService
    {
        ...
        public AdminService()
        {
            Program.BotEvents.OnBanEvent += OnBanEvent;
            Program.BotEvents.OnBitEvent += OnBitEvent;
            Program.BotEvents.OnTwitchChannelGoesLive += OnChannelGoesLive;
            Program.BotEvents.OnTwitchChannelGoesOffline += OnChannelGoesOffline;
            Program.BotEvents.OnHostEvent += OnHostEvent;
            Program.BotEvents.OnRaidEvent += OnRaidEvent;
            Program.BotEvents.OnDiscordNewMember += OnNewDiscordMember;
            Program.BotEvents.OnUnBanEvent += OnUnBanEvent;
            Program.BotEvents.OnTwitchSubEvent += OnTwitchSub;
            Program.BotEvents.OnUserEntryMerge += OnUserEntryMergeEvent;
            Program.BotEvents.OnDiscordUserStartStream += OnDiscordUserStartStreaming;
        }
                ...
                case "announcestream":
                    if (args.Count > 1)
                    {
                        if (args[1].ToLower() == "debug")
                        {
                            ...
                            UserEntry user = await Program.Users.GetUserByDiscordID(Context.User.Id);
                            StreamingGame streamInfo = new StreamingGame("Twitch", $"http://twitch.com/{user._twitchUsername}");
                           
                            Program.BotEvents.RaiseDiscordUserStartStream(
                                bChan,
                                user,
                                streamInfo
                                );
        ...
        {
            List<BotChannel> channels = await Program.Channels.GetChannels();
            if (!channels.Exists(p => p.GuildID == context.Guild.Id) && !channels.Exists(p => p.TwitchChannelName == twitchChannelName))
            {
                return;
            }
            BotChannel discordProfile = await Program.Channels.GetDiscordGuildbyID(context.Guild.Id);
            if (discordProfile.isLinked || (await Program.Channels.GetTwitchChannelByName(twitchChannelName)).isLinked)
            {
                return;
            }
            BotChannel linkedProfile = new BotChannel(context.Guild.Id,  context.Guild.Name);
            BotChannel twitchProfile = await Program.Channels.GetTwitchChannelByName(twitchChannelName);
            ...
            bool result = await Program.Channels.SaveAsLinked(linkedProfile);
            // Trigger OnLinkingChannel event if channel was added
            if (result)
            {
                Program.BotEvents.RaiseOnBotChannelMerge(discordProfile, twitchProfile);
                ...
                Program.Channels.ChannelSave(linkedProfile);
            }else
            {
        ...
        {
            if(await Program.Channels.JoinTwitchChannel(twitchChannelName))
            {
        ...
        {
            List<UserEntry> matches = await Program.Users.SearchUserName(search);
